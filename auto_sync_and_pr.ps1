...
function Get-ExistingPR {
    ...
    try {
        ...
        $prs = gh pr list --state open --json number,headRefName,title 2>$null
        
        if ($LASTEXITCODE -ne 0) {
            Write-Log "Failed to get PR list from GitHub CLI" -Level "WARNING"
            return $null
        }
        
        if (-not $prs -or $prs.Trim() -eq '') {
        ...
        if ($matchingPRs) {
            ...
            Write-Log "Found existing PR #$($latestPR.number) on branch $($latestPR.headRefName)" -Level "INFO"
